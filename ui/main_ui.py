            ...
            if len(st.session_state.chat_history) > 3:
                ...
                if st.button("✨ Turn this conversation into structured Requirements", type="primary", width="stretch"):
                    ...
                    if not ok:
                        ...
                        # Step 1: Database — runs independently; all other steps use req as-is if this fails
                        with st.spinner("⚙️ Running database agent… (1/5)"):
                            try:
                                db_agent = DatabaseAgent(api_key)
                                schema_result = db_agent.generate_schema(req)
                                if "error" not in schema_result:
                                    ...
                                    step_errors["database"] = schema_result.get("error", "Database schema failed")
                            except Exception as e:
                                step_errors["database"] = str(e)[:200]

                        # Step 2: Design — independent; does not require database schema
                        with st.spinner("⚙️ Running design agent… (2/5)"):
                            try:
                                design_agent = DesignGenerationAgent(api_key)
                                design = design_agent.generate_design(req)
                                st.session_state.interface_design = design
                                ...
                                if "error" not in design:
                                    ...
                                    step_errors["design"] = design.get("error", "Design failed")
                            except Exception as e:
                                step_errors["design"] = str(e)[:200]

                        # Step 3: Planning — uses design if available, otherwise runs with requirements only
                        with st.spinner("⚙️ Running planning agent… (3/5)"):
                            try:
                                plan_agent = PlanningManagerAgent(api_key)
                                plan = plan_agent.generate_cycle_plan(
                                    req,
                                    st.session_state.get("interface_design"),
                                )
                                st.session_state.cycle_plan = plan
                                if "error" not in plan:
                                    ...
                                    step_errors["planning"] = plan.get("error", "Planning failed")
                            except Exception as e:
                                step_errors["planning"] = str(e)[:200]

                        # Step 4: Quality — db_schema and design are optional
                        with st.spinner("⚙️ Running quality agent… (4/5)"):
                            try:
                                quality_agent = QualityManagerAgent(api_key)
                                db_schema = req.get("database_schema")
                                report = quality_agent.generate_quality_report(
                                    req, st.session_state.get("interface_design"), db_schema
                                )
                                st.session_state.quality_report = report
                                if "error" not in report:
                                    ...
                                    step_errors["quality"] = report.get("error", "Quality check failed")
                            except Exception as e:
                                step_errors["quality"] = str(e)[:200]

                        # Step 5: Support — all inputs are optional beyond requirements
                        with st.spinner("⚙️ Running support agent… (5/5)"):
                            try:
                                support_agent = SupportManagerAgent(api_key)
                                db_schema = req.get("database_schema")
                                pkg = support_agent.generate_support_package(
                                ...
                                if "error" not in pkg:
                                    ...
                                    step_errors["support"] = pkg.get("error", "Support package failed")
                            except Exception as e:
                                step_errors["support"] = str(e)[:200]

                        # Show per-step errors as warnings (not fatal — other steps still ran)
                        for step_name, err_msg in step_errors.items():
                            st.warning(f"⚠️ {step_name.capitalize()} step failed: {err_msg}")
                        if completed:
                            st.success(f"✅ Done! Completed: {', '.join(completed)}")
